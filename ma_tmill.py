...
if st.button("🚀 Merge PDFs"):
    ...
    for idx, row in input_df.iterrows():
        try:
            ...
            if not (memo_no or bill_no):
                logs.append(f"Row {idx}: ❌ Missing Memo & Bill No")
                continue

            if not einv_no or not rename:
                logs.append(f"Row {idx}: ❌ Missing E-Invoice or Rename value")
                continue
            ...
            if not invoice_pdf:
                logs.append(f"Row {idx}: ❌ Invoice PDF not found")
                continue

            if not einv_pdf:
                logs.append(f"Row {idx}: ❌ E-Invoice PDF not found")
                continue
            ...
            logs.append(f"Row {idx}: ✅ Created {rename}.pdf")
    ...
    st.download_button(
        label="⬇️ Download Merged PDFs (ZIP)",
